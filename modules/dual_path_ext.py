...
class Dual_Computation_Block(nn.Module):
    ...
    def __init__(
        ...
        cond_dim=768,
        film_before_intra=False,
        film_before_inter=False,
        film_n_layer=2,
        film_scale=True
    ):
        ...
        # FiLM
        if film_before_intra:
            self.intra_film = FiLM(
                in_dim=cond_dim,
                out_dim=out_channels,
                n_layer=film_n_layer,
                scale=film_scale
            )
            print(f'Initialized a FiLM before a IntraTransformer.')
        self.film_before_intra = film_before_intra

        if film_before_inter:
            self.inter_film = FiLM(
                in_dim=cond_dim,
                out_dim=out_channels,
                n_layer=film_n_layer,
                scale=film_scale
            )
            print(f'Initialized a FiLM before a InterTransformer.')
        self.film_before_inter = film_before_inter
    ...
    def forward(self, x, cond_embed=None):
        ...
        if cond_embed != None and self.film_before_intra:
            intra = self.intra_film(
                intra.permute(0, 2, 1).contiguous(), 
                cond_embed
        ...
        if cond_embed != None and self.film_before_inter:
            inter = self.inter_film(
                inter.permute(0, 2, 1).contiguous(), 
                cond_embed
            ).permute(0, 2, 1).contiguous(), 
...
class Dual_Path_Model(nn.Module):
    ...
    def __init__(
            ...
            self.dual_mdl.append(
                copy.deepcopy(
                    Dual_Computation_Block(
                        ...
                        cond_dim=cond_dim,
                        film_before_intra=(film_mode=='iblock'),
                        film_before_inter=(film_mode=='iblock'),
        ...
        assert film_mode in ['none', 'block', 'iblock']
        ...
        if film_mode == 'block':
            ...
            for i in range(num_layers):
                ...
                print(f'Initialized a FiLM before IntraInterBlock {i}.')
        self.film_mode = film_mode

        self.analyze = analyze
        if analyze:
            print('### Analyze mode: MaskNet returns output and features ###')

        assert num_layers == 2

    def forward(self, x, cond_embed=None, features=None):
        """Returns the output tensor.
        ...
        if self.analyze:
            features['Block0 Input Before FiLM (2D)'] = x.detach().clone().squeeze()

        # [B, N, K, S]
        x, gap = self._Segmentation(x, self.K)

        if self.analyze:
            features['Block0 Input Before FiLM (3D)'] = x.detach().clone().squeeze()

        # [B, N, K, S]
        x = self.films[0](x, cond_embed, features, 0)

        if self.analyze:
            features['Block0 Input After FiLM (2D)'] = self._over_add(x.detach().clone(), gap).squeeze()
            features['Block0 Input After FiLM (3D)'] = x.detach().clone().squeeze()

        x = self.dual_mdl[0](x, cond_embed) # cond_embed here is dummy

        if self.analyze:
            features['Block1 Input Before FiLM (2D)'] = self._over_add(x.detach().clone(), gap).squeeze()
            features['Block1 Input Before FiLM (3D)'] = x.detach().clone().squeeze()

        x = self.films[1](x, cond_embed, features, 1)

        if self.analyze:
            features['Block1 Input After FiLM (2D)'] = self._over_add(x.detach().clone(), gap).squeeze()
            features['Block1 Input After FiLM (3D)'] = x.detach().clone().squeeze()

        x = self.dual_mdl[1](x, cond_embed) # cond_embed here is dummy

        if self.analyze:
            features['Block1 Output (2D)'] = self._over_add(x.detach().clone(), gap).squeeze()
            features['Block1 Output (3D)'] = x.detach().clone().squeeze()
